...
Start: **M1.1–M1.3** (a build you can open from Applications — M1.1, M1.2 done 2026-09-23; M1.3 built, the type registers as a package, the double-click not yet checked in the installed build; CI workflows written, no tag pushed yet), then M1.4 (the recovery journal), then the M1.9 benchmark and M1.10 (CI publishing to GitHub Releases). D1–D3 are decided (§4).
